...
public:
    sdl_window_test_helper()
    {
        ...
        m_window = SDL_CreateWindow("TestGLESWindow", 0, 0, 1, 1, SDL_WINDOW_HIDDEN | SDL_WINDOW_OPENGL);
        if (!m_window)
        {
            Log("~ Cannot create helper window for OpenGLES:", SDL_GetError());
            return;
        }
        ...
        if (!m_context)
        {
            Log("~ Cannot create GLES context:", SDL_GetError());
            return;
        }
    }
...
XR_EXPORT bool CheckRendererSupport()
{
    ...
    {
        Log("~ No shaders found for OpenGLES");
        return false;
    }
