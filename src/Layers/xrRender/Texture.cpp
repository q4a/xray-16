...
#if defined(VV2)//(XR_PLATFORM_WINDOWS)
#include <gli/gli.hpp>

typedef D3DVECTOR Vect3f;
...
Vect3f normalizeColor(DWORD d)
{
    ...
    uint8_t y = (d>> 8) & 0xFF;
    uint8_t z = (d) & 0xFF;
    v.x = static_cast<float>(x) * (1/127.5f)-1;
    v.y = static_cast<float>(y) * (1/127.5f)-1;
    v.z = static_cast<float>(z) * (1/127.5f)-1;
    //v-=Vect3f(1,1,1);
    return v;
}
    ...
    for (c = 0; c < 4; ++c)
    {
        ...
        if (format->bits[c])
        {
            ...
            if (format->type == FORMAT_ARGBF16) {
                Msg("Unimplemented for FORMAT_ARGBF16");
...
#if !defined(XR_PLATFORM_WINDOWS)//(XR_PLATFORM_WINDOWS)
#include <gli/gli.hpp>
